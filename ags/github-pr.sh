...
if [ "$pr_review_requests_count" == "0" ]; then
  ...
  pr_review_requests="$(echo $github_status_output | grep -oE PRAVALER\/\[^#\]+#\[0-9\]\{1,4\}\[^│]+ | sed 's/\(PRAVALER\/[^#]\+#[0-9]\+\)\s\+\(.*$\)/\r\1 \2/')"
  alt="pending"
fi
...
# print_output
echo -e {\"count\": $pr_review_requests_count, \"prLists\": \"$pr_review_requests\"}
